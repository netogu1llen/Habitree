document.addEventListener("click", (event) => {
    ...
    fetch(`/leagues/add-modal`)
        // Verifica que la respuesta sea correcta
        .then(res => {
            if (!res.ok) {
                throw new Error("Error al obtener el modal de agregar league");
            }
            return res.text();
        })
        ...
        .catch(err => {
            console.error(err);
            alert("There was an error loading the add league modal. Please try again.");
        });
});